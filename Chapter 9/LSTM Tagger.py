# -*- coding: utf-8 -*-
"""
LSTM POS Tagger.ipynb
Implementation of a LSTM based POS Tagger.
"""
...
vocab = Vocab()
LSTMTagger = LSTM_POS_Tagger(vocab.n_words, 256, vocab.n_tags).to(device)
BiLSTMTagger = BiLSTM_POS_Tagger(vocab.n_words, 256, vocab.n_tags).to(device)

trainIter(LSTMTagger, lr = 0.5, epochs = 10, print_every = 4000, is_validate = False)
trainIter(BiLSTMTagger, lr = 0.5, epochs = 10, print_every = 4000, is_validate = False)
...
# Evaluating LSTM over testing dataset.  Acheived accuracy ~89%
...
for sample in test_iter:
  accuracy_track.append(evaluate(sample, LSTMTagger, False))
  num_samples -= 1
  if num_samples <=0:
    break

mean_accuracy = np.array(accuracy_track).mean()
print(f"Mean accuracy of LSTMTagger over {num_samples_copy} samples is {mean_accuracy}%")

# Evaluating BiLSTM over testing dataset. Acheived accuracy ~90%
test_iter = datasets.CoNLL2000Chunking(split='test')
num_samples = len(test_iter)
num_samples_copy = num_samples
accuracy_track = []
...
for sample in test_iter:
  accuracy_track.append(evaluate(sample, BiLSTMTagger, False))
  num_samples -= 1
  if num_samples <=0:
    break

mean_accuracy = np.array(accuracy_track).mean()
print(f"Mean accuracy of BiLSTMTagger over {num_samples_copy} samples is {mean_accuracy}%")